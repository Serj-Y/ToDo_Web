...
export const UpdateToDoList = ({
    ...
    const onSubmit = useCallback(async (data: FormData) => {
        await dispatch(updateToDoList({ name: data.name, todoId: toDoId })).then(() => setIsEditToDoList(false));
    }, [dispatch, setIsEditToDoList, toDoId]);
    return (
        <form onSubmit={handleSubmit(onSubmit)} className={cls.UpdateToDoList}>
            <Controller
                name="name"
                control={control}
